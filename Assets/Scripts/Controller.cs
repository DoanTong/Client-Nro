...
public class Controller : IMessageHandler
{
		...
		try
		{
			...
			case -95:
			{
				...
				if (type4 == 3)
				{
					...
					if (@char != null)
					{
						...
						if (mob != null)
						{
							...
							{
								GameScr.startFlyText("-" + NinjaUtil.getMoneys(dame), mob.x, mob.y - mob.h, 0, -2, mFont.ORANGE);
							}
						}
					}
				}
				...
				if (type4 == 5)
				{
					...
					{
						GameScr.startFlyText("-" + NinjaUtil.getMoneys(num250), mob9.x, mob9.y - mob9.h, 0, -2, mFont.ORANGE);
					}
				}
				if (type4 == 6)
				{
			...
			case -93:
			{
				int num51 = msg.reader().readUnsignedShort();
				// Server gửi danh sách BgItem.id, không phải checksum PNG.
			...
			case -74:
			{
				...
				if (b38 == 0)
				{
					// Server chỉ gửi 4 byte resource version, không gửi UTF key
					int serverVersion = msg.reader().readInt();

					string resVersionKey =
						"ResVersion_srv"
					...
					int cachedVersion = -1;
					string cachedText = Rms.loadRMSString(resVersionKey);

					if (!string.IsNullOrEmpty(cachedText))
					{
						int.TryParse(cachedText, out cachedVersion);
					}

					// Resource server là PNG thường
					ServerListScreen.keyDecryptString = string.Empty;
					ModFunc.isEncryptIcon = false;
					...
					Debug.Log(
						...
						+ ", key=" + resVersionKey
						+ ", remaining=" + msg.reader().available()
					);
					...
					if (Session_ME.gI().isCompareIPConnect())
					{
						...
						{
							// Không cho dữ liệu ảnh cũ tiếp tục được sử dụng
							BgItem.clearHashTable();
							SmallImage.clearAllIconCache();
				...
				if (b38 == 3)
				{
					...
					ModFunc.InitMusic();
					isLoadingData = false;
					int downloadedResVersion = msg.reader().readInt();
					string downloadedResKey =
						"ResVersion_srv" + ServerListScreen.ipSelect
						+ "_x" + mGraphics.zoomLevel;
					Rms.saveRMSString(
						downloadedResKey,
						downloadedResVersion + string.Empty
					);
					Rms.saveRMSString(
						"ResVersion",
						downloadedResVersion + string.Empty
				...
				if (b38 == 5)
				{
					try
					{
						...
						// FarmAssetManager của source này nhận dữ liệu PNG dạng sbyte[]
			...
			case -65:
			{
				...
				int playerId = msg.reader().readInt();
				sbyte teleportType = msg.reader().readByte();

				Debug.Log(
					"[PACKET -65]"
					+ ", target=" + playerId
					+ ", myId=" + Char.myCharz().charID
					+ ", type=" + teleportType
					+ ", map=" + TileMap.mapID
					+ ", stopBefore=" + Controller.isStopReadMessage
					+ ", lockBefore=" + GameScr.lockTick
					+ ", loading=" + Char.isLoadingMap
				);
				...
				{
					Char other = GameScr.findCharInMap(playerId);
					if (other == null)
					{
						break;
					}
			...
			case -67:
			{
				...
				try
				{
					// Server gửi: int id + int length + byte[] PNG.
					// Không có UTF key và không mã hóa.
					sbyte[] iconData = NinjaUtil.readByteArray(msg);
					Image iconImage = Image.createImage(
					...
					{
						throw new Exception(
							"Không tạo được PNG, length=" + iconData.Length
						);
					}

					SmallImage.ensureCapacity(iconId);
					if (SmallImage.imgNew[iconId] == null)
					{
						SmallImage.imgNew[iconId] =
							new Small(iconImage, iconId);
					}
					else
					{
						SmallImage.imgNew[iconId].img = iconImage;
					}

					SmallImage.imageRaw[iconId] = iconImage;
					SmallImage.saveIconToRMS(iconId, iconData);
				}
				catch (Exception ex)
				{
					Debug.LogError(
						"[ICON] Nhận icon lỗi, id=" + iconId
						+ ", remaining=" + msg.reader().available()
						+ "\n" + ex
					);
				}
			...
			case -32:
			{
				...
				{
					string bgCacheKey =
						"srv" + ServerListScreen.ipSelect
						+ "_x" + mGraphics.zoomLevel
						+ "_bgItem" + num228;
					Rms.saveRMS(bgCacheKey, array13);
			...
			case -26:
			{
                ...
                if (msgDlg == "Vui lòng mở giới hạn sức mạnh" || msgDlg == "Bạn không đủ tiềm năng" || msgDlg == "")
                {
                    ModFunc.indexAutoPoint = -1;
                    ModFunc.pointIncrease = 0;
                    ModFunc.autoPointForPet = false;
                    if (msgDlg == "Vui lòng mở giới hạn sức mạnh")
                    {
                        GameScr.info1.addInfo("Chỉ số đã đạt tối đa", 0);
                    }
					else
					{
                        GameScr.info1.addInfo(msgDlg, 0);
                    }
					break;
                }
			...
			case 56:
			{
				...
				if (charID == Char.myCharz().charID)
				{
					...
					if (Char.myCharz().cTypePk != 4)
					{
						...
						{
							GameScr.startFlyText("-" + NinjaUtil.getMoneys(dameHit), @char.cx, @char.cy - @char.ch, 0, -3, flag9 ? mFont.FATAL : mFont.RED);
						}
					}
					break;
				}
				...
				if (@char.cTypePk != 4)
				{
					...
					{
						GameScr.startFlyText("-" + NinjaUtil.getMoneys(dameHit2), @char.cx, @char.cy - @char.ch, 0, -3, flag10 ? mFont.FATAL : mFont.ORANGE);
					}
				}
				break;
			}
			...
			case -20:
			{
				...
				for (int num213 = 0; num213 < GameScr.vItemMap.size(); num213++)
				{
					...
					if (text6.Equals(string.Empty))
					{
						...
						{
							GameScr.startFlyText(((num >= 0) ? "+" : string.Empty) + NinjaUtil.getMoneys(num), Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch, 0, -2, mFont.RED);
							SoundMn.gI().getItem();
						}
						else
						{
			...
			case -24:
			{
				...
				{
					Debug.LogWarning(
						"[MAP] Bo qua packet -24 trung"
						+ ", currentMapID=" + TileMap.mapID
						+ ", pending=" + messWait.reader().available()
					);
					break;
				}
				...
				{
					int pendingLength = msg.reader().available();
					sbyte[] pendingData = new sbyte[pendingLength];
					msg.reader().readFully(ref pendingData);

					messWait = new Message((sbyte)(-24), pendingData);

					Debug.Log(
						"[MAP] Khong co map local, request template"
						+ ", mapID=" + TileMap.mapID
			...
			}case -31:
			{
				...
				int num176 = msg.reader().readUnsignedShort();
				for (int num177 = 0; num177 < num176; num177++)
				{
					...
					int b51 = msg.reader().readUnsignedByte();
					bgItem.tileX = new int[b51];
					bgItem.tileY = new int[b51];
					for (int num178 = 0; num178 < b51; num178++)
					{
						bgItem.tileX[num178] = msg.reader().readUnsignedByte();
						bgItem.tileY[num178] = msg.reader().readUnsignedByte();
					}
			...
			case -2:
			{
				...
				GameScr.startFlyText((num282 <= 0) ? (string.Empty + NinjaUtil.getMoneys(num282)) : ("+" + NinjaUtil.getMoneys(num282)), Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch - 10, 0, -2, mFont.YELLOW);
				break;
			}
			case 95:
			{
				...
				Char.myCharz().xuStr = mSystem.numberTostring(Char.myCharz().xu);
				GameScr.startFlyText((num296 <= 0) ? (string.Empty + NinjaUtil.getMoneys(num296)) : ("+" + NinjaUtil.getMoneys(num296)), Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch - 10, 0, -2, mFont.YELLOW);
				break;
			}
			case 96:
			...
			case -1:
			{
				...
				GameScr.startFlyText("+" + NinjaUtil.getMoneys(num286), Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch - 10, 0, -2, mFont.YELLOW);
				break;
			}
			case -3:
			{
                ...
                { // chieu.lq +tiemnang startFlyText
                    GameScr.startFlyText(((param11 <= 0.0) ? string.Empty : "+") + NinjaUtil.getMoneys(param11), Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch, 0, -4, mFont.GREEN);
			...
			case -9:
			{
				...
				if (dame2 != 1.0)
				{
					...
					{
						GameScr.startFlyText("-" + NinjaUtil.getMoneys(dame2), mob11.x, mob11.getY() - mob11.getH(), 0, -2, mFont.ORANGE);
					}
				}
				break;
			}
			...
			case -12:
			{
				...
				try
				{
					...
					{
						GameScr.startFlyText("-" + NinjaUtil.getMoneys(dameHit4), mob16.x, mob16.y - mob16.h, 0, -2, mFont.FATAL);
					}
					else
					{
						GameScr.startFlyText("-" + NinjaUtil.getMoneys(dameHit4), mob16.x, mob16.y - mob16.h, 0, -2, mFont.ORANGE);
					}
    ...
    private void createMap(myReader d)
    {
        ...
        int mapCount = d.readUnsignedByte();
        ...
        int npcCount = d.readUnsignedByte();
        Npc.arrNpcTemplate = new NpcTemplate[npcCount];
        for (int i = 0; i < npcCount; i++)
        {
            Npc.arrNpcTemplate[i] = new NpcTemplate();
            Npc.arrNpcTemplate[i].npcTemplateId = i;
            Npc.arrNpcTemplate[i].name = d.readUTF();
            Npc.arrNpcTemplate[i].headId = d.readShort();
            Npc.arrNpcTemplate[i].bodyId = d.readShort();
            Npc.arrNpcTemplate[i].legId = d.readShort();

            int menuGroupCount = d.readUnsignedByte();
            Npc.arrNpcTemplate[i].menu = new string[menuGroupCount][];
            for (int j = 0; j < menuGroupCount; j++)
            {
                int menuItemCount = d.readUnsignedByte();
                Npc.arrNpcTemplate[i].menu[j] =
                    new string[menuItemCount];

                for (int k = 0; k < menuItemCount; k++)
                {
                    Npc.arrNpcTemplate[i].menu[j][k] = d.readUTF();
                }
            }
        }

        int mobCount = d.readUnsignedByte();
        Mob.arrMobTemplate = new MobTemplate[mobCount];
        for (int i = 0; i < mobCount; i++)
        {
            Mob.arrMobTemplate[i] = new MobTemplate();
            Mob.arrMobTemplate[i].mobTemplateId = (sbyte)i;	
            Mob.arrMobTemplate[i].type = d.readByte();
            Mob.arrMobTemplate[i].name = d.readUTF();
            Mob.arrMobTemplate[i].hp = d.readIntToLongDQT();
            Mob.arrMobTemplate[i].rangeMove = d.readByte();
            Mob.arrMobTemplate[i].speed = d.readByte();
            Mob.arrMobTemplate[i].dartType = d.readByte();
        }

        Debug.Log(
            "[MAP DATA] version=" + GameScr.vcMap
            + ", maps=" + mapCount
            + ", npcs=" + npcCount
            + ", mobs=" + mobCount
            + ", remaining=" + d.available()
        );
    }
   ...
   public void loadCurrMap(sbyte teleport3)
		{
			...
			Char.myCharz().isPaintNewSkill = false;
			...
			for (int i = 0; i < Char.myCharz().vEff.size(); i++)
			{
				EffectChar effectChar =
					(EffectChar)Char.myCharz().vEff.elementAt(i);

				if (effectChar != null
					&& effectChar.template != null
					&& effectChar.template.type == 10)
				{
					Char.isLockKey = true;
					break;
				}
			...
			{
				if (TileMap.mapID == GameScr.gI().mapRID
					&& TileMap.zoneID == GameScr.gI().zoneRID)
				{
					GameScr.gI().callRongThan(
						GameScr.gI().xR,
						GameScr.gI().yR
					);
				}
			...
			InfoDlg.show(
				TileMap.mapName,
				mResources.zone + " " + TileMap.zoneID,
				30
			);
			...
			Debug.Log(
				"[MAP READY]"
				+ ", mapID=" + TileMap.mapID
				+ ", char=" + Char.myCharz().cx + "," + Char.myCharz().cy
				+ ", camera=" + GameScr.cmx + "," + GameScr.cmy
				+ ", stopRead=" + Controller.isStopReadMessage
				+ ", lockTick=" + GameScr.lockTick
				+ ", loading=" + Char.isLoadingMap
				+ ", teleport=" + Char.myCharz().isTeleport
			);
		}
    public void loadInfoMap(Message msg)
	{
				...
				for (int m = 0; m < num6; m++)
				{
					...
					if (TileMap.getBIById(num7) != null)
					{
						...
						if (!BgItem.imgNew.containsKey(bgItem.idImage + string.Empty))
						{
							if (mGraphics.zoomLevel == 1)
							{
								// Không dùng ảnh local của client sưu tầm.
							...
							else
							{
								...
								string bgCacheKey =
									"srv" + ServerListScreen.ipSelect
									+ "_x" + mGraphics.zoomLevel
									+ "_bgItem" + bgItem.idImage;
								sbyte[] array = Rms.loadRMS(bgCacheKey);
		...
		{
			int position =
				(msg != null && msg.reader() != null)
					? msg.reader().posRead
					: -1;
			int remaining =
				(msg != null && msg.reader() != null)
					? msg.reader().available()
					: -1;
            ...
            switch (b)
            {
				...
				GameScr.vcItem = -1; 
                    {
						...
						// TẠM THỜI XÓA CACHE DATA CŨ
						Rms.DeleteStorage("NR_dart");
						Rms.DeleteStorage("NR_arrow");
						Rms.DeleteStorage("NR_effect");
						Rms.DeleteStorage("NR_image");
						Rms.DeleteStorage("NR_part");
						Rms.DeleteStorage("NR_skill");
						Rms.DeleteStorage("NRdata");

						// Ép client tải lại data từ server
						GameScr.vcData = -1;
						GameScr.isLoadAllData = false;

						Debug.Log("[CACHE] Đã xóa cache DATA và PART cũ");
                        sbyte b3 = msg.reader().readByte();
                        ...
                        Debug.Log(
                            "[CLIENT] Server versions - vsData=" + GameScr.vsData
                            + ", vsMap=" + GameScr.vsMap
                            + ", vsSkill=" + GameScr.vsSkill
                            + ", vsItem=" + GameScr.vsItem
                        );

                        Debug.Log(
                            "[CLIENT] Cached versions - vcData=" + GameScr.vcData
                            + ", vcMap=" + GameScr.vcMap
                            + ", vcSkill=" + GameScr.vcSkill
                            + ", vcItem=" + GameScr.vcItem
                        );

                        Debug.Log(
                            "[CLIENT] Item version check: vsItem("
                            + GameScr.vsItem + ") != vcItem("
                            + GameScr.vcItem + ") = "
                            + (GameScr.vsItem != GameScr.vcItem)
                        );

                        // TẠM THỜI: ép tải lại item template để bỏ cache cũ.
                        // Sau khi log báo đã tải và lưu cache thành công, có thể xóa dòng này.
                        GameScr.vcItem = -1;

                        if (GameScr.vsItem != GameScr.vcItem)
                        {
                            Debug.Log(
                                "[CLIENT] Item version mismatch! "
                                + "Will request new item templates from server."
                            );
                        ...
                        else
                        {
                            try
                            {
                                // Server này chỉ dùng item subtype 0, 1, 2 và 100.
                                // Không đọc NRitem101 và không dùng cache legacy NRitem1/NRitem2.
                                sbyte[] itemOptionData = Rms.loadRMS("NRitem0");
                                if (itemOptionData == null || itemOptionData.Length == 0)
                                {
                                    throw new Exception("Thiếu cache NRitem0");
                                }
                                loadItemNew(new myReader(itemOptionData), 0, false);

                                if (!loadItemTemplatesCache())
                                {
                                    throw new Exception("Thiếu hoặc lỗi cache NRitemCache");
                                }

                                sbyte[] arrHeadData = Rms.loadRMS("NRitem100");
                                if (arrHeadData == null || arrHeadData.Length == 0)
                                {
                                    throw new Exception("Thiếu cache NRitem100");
                                }
                                loadItemNew(new myReader(arrHeadData), 100, false);

                                LoginScr.isUpdateItem = false;
                                Debug.Log(
                                    "[CLIENT] Item cache loaded successfully, total="
                                    + ItemTemplates.count()
                                );
                            }
                            catch (Exception ex)
                            {
                                Debug.LogError(
                                    "[CLIENT] Error loading item cache: " + ex
                                );

                                // Ép lệch version để tải lại đầy đủ từ server.
                                GameScr.vcItem = -1;
                                GameScr.isLoadAllData = false;
                                Service.gI().updateItem();
                            }
                ...
                case 10:
				{
					...
					try
					{
						...
						int mapWidth = msg.reader().readUnsignedByte();
						int mapHeight = msg.reader().readUnsignedByte();

						if (mapWidth <= 0 || mapHeight <= 0)
						{
							throw new Exception(
								"Kich thuoc map khong hop le: "
								+ mapWidth + "x" + mapHeight
							);
						}

						int tileCount = mapWidth * mapHeight;
						int remainingBeforeTiles = msg.reader().available();

						if (remainingBeforeTiles < tileCount)
						{
							throw new Exception(
								"Thieu du lieu tile: can="
								+ tileCount
								+ ", con="
								+ remainingBeforeTiles
							);
						}

						TileMap.tmw = mapWidth;
						TileMap.tmh = mapHeight;
						TileMap.maps = new int[tileCount];
						TileMap.types = new int[tileCount];

						sbyte[] cacheBuf = new sbyte[2 + tileCount];
						cacheBuf[0] = (sbyte)mapWidth;
						cacheBuf[1] = (sbyte)mapHeight;

						for (int i = 0; i < tileCount; i++)
						{
							byte tile = msg.reader().readUnsignedByte();
							TileMap.maps[i] = tile;
							cacheBuf[2 + i] = (sbyte)tile;
						}

						templateLoaded = true;

						Rms.saveRMS(
							"NRmaptile_" + TileMap.mapID,
							cacheBuf
						);
						...
						Debug.Log(
							"[MAP] Loaded template"
							+ ", mapID=" + TileMap.mapID
							+ ", tileID=" + TileMap.tileID
							+ ", size=" + mapWidth + "x" + mapHeight
							+ ", tiles=" + tileCount
							+ ", extra=" + msg.reader().available()
						);

						Message pendingMapMessage = messWait;

						if (pendingMapMessage == null)
						{
							Debug.LogWarning(
								"[MAP] Bo qua template response du"
								+ ", mapID=" + TileMap.mapID
								+ ", size=" + mapWidth + "x" + mapHeight
							);
						...
						loadInfoMap(pendingMapMessage);

						try
						{
							TileMap.isMapDouble =
								pendingMapMessage.reader().readByte() != 0;
						}
						catch (Exception)
						{
							TileMap.isMapDouble = false;
						...
						Debug.Log(
							...
							+ ", lockTick=" + GameScr.lockTick
							+ ", loading=" + Char.isLoadingMap
							+ ", queue=" + Session_ME.recieveMsg.size()
						);
					}
					catch (Exception ex)
					{
						Debug.LogError(
							"[MAP] REQUEST_MAPTEMPLATE loi"
							+ ", mapID=" + TileMap.mapID
							+ ", remaining="
							+ ((msg != null && msg.reader() != null)
								? msg.reader().available()
...
// 	}
	private void loadItemNew(myReader d, sbyte type, bool isSave)
{
    if (d == null || d.buffer == null || d.buffer.Length == 0)
    {
        Debug.LogWarning(
            "[CLIENT] Bỏ qua item data rỗng, requestedType=" + type
        );
        return;
    }
    ...
    try
    {
        ...
        switch (type)
        {
            case 0:
            {
                // Server ghi bằng writeByte(), phải đọc unsigned byte.
                int optionCount = d.readUnsignedByte();

                GameScr.gI().iOptionTemplates =
                    new ItemOptionTemplate[optionCount];

                for (int i = 0; i < optionCount; i++)
                {
                    ItemOptionTemplate option = new ItemOptionTemplate();
                    option.id = i;
                    option.name = d.readUTF();
                    option.type = d.readByte();

                    GameScr.gI().iOptionTemplates[i] = option;
                }
                ...
                if (isSave)
                {
                    ...
                    sbyte[] optionData = new sbyte[d.available()];
                    d.readFully(ref optionData);

                    Rms.saveRMS("NRitem0", optionData);
                }
            ...
            case 1:
            {
                ...
                // Server gửi số lượng item từ ID 0.
                int count = d.readUnsignedShort();

                Debug.Log(
                    "[CLIENT] Nhận item batch 1, count=" + count
                );

                for (int itemId = 0; itemId < count; itemId++)
                {
                    ItemTemplate item = readItemTemplate(
                        d,
                        (short)itemId
                    );
            ...
            case 2:
            {
                // Server gửi khoảng ID [start, end).
                int start = d.readUnsignedShort();
                int end = d.readUnsignedShort();

                Debug.Log(
                    "[CLIENT] Nhận item batch 2: start="
                    + start
                    + ", end="
                    + end
                );
                ...
                Debug.Log(
                    "[CLIENT] Tổng item đã tải: "
                    + ItemTemplates.count()
                );

                // Theo ItemData.java, case 2 luôn là packet item cuối.
                if (isSave)
                {
                    saveItemTemplatesCache();
                    ...
                    Rms.saveRMS(
                        "NRitemVersion",
                        new sbyte[] { GameScr.vcItem }
                    );

                    LoginScr.isUpdateItem = false;

                    bool allVersionMatched =
                        GameScr.vsData == GameScr.vcData
                        && GameScr.vsMap == GameScr.vcMap
                        && GameScr.vsSkill == GameScr.vcSkill
                        && GameScr.vsItem == GameScr.vcItem;

                    if (allVersionMatched)
                    {
                        GameScr.gI().readDart();
                        GameScr.gI().readEfect();
                        GameScr.gI().readArrow();
            ...
            case 100:
            {
                ...
                if (isSave)
                {
                    ...
                    sbyte[] headData = new sbyte[d.available()];
                    d.readFully(ref headData);

                    Rms.saveRMS("NRitem100", headData);
                }
    ...
    {
        int position = (d != null) ? d.posRead : -1;
        int remaining =
            (d != null && d.buffer != null) ? d.available() : -1;

        Debug.LogError(
            "[CLIENT] loadItemNew lỗi"
            + ", type=" + type
            + ", pos=" + position
            + ", remaining=" + remaining
            + "\n" + ex
...
{
    sbyte itemType = reader.readByte();
