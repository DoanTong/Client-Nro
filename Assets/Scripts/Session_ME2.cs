...
public class Session_ME2 : ISession
{
    ...
    public class Sender
    {
        ...
        public void run()
        {
            while (connected)
            {
                ...
                {
                    Debug.LogError("Session_ME2 sender error: " + ex);
                    Thread.Sleep(20);
                }
            }
        }
    ...
    private class MessageCollector
    {
        public void run()
        {
            ...
            {
                Debug.LogError("Session_ME2 collector error: " + ex);
                shouldNotifyDisconnect = !isCancel;
            }
            finally
            {
        ...
        private void getKey(Message message)
        {
            try
            {
                ...
                // Session phụ chỉ cần key; các trường IP2/PORT2 đã do session chính quản lý.

            }
            catch (Exception ex)
            {
                Debug.LogError("Session_ME2 getKey error: " + ex);
                connected = false;
            }
        }
        ...
        private Message readMessage()
        {
            ...
            catch (Exception ex)
            {
                if (!isCancel)
                {
                    Debug.LogError("Session_ME2 readMessage error: " + ex.Message);
                }
    ...
    private void NetworkInit()
    {
        ...
        catch (Exception ex)
        {
            if (!isCancel)
            {
                Debug.LogError("Session_ME2 connect error: " + ex.Message);
            }
    ...
    public static sbyte readKey(sbyte value)
    {
        ...
        {
            throw new InvalidOperationException("Session_ME2 read key chưa sẵn sàng");
        }
    ...
    public static sbyte writeKey(sbyte value)
    {
        if (key == null || key.Length == 0)
        {
            throw new InvalidOperationException("Session_ME2 write key chưa sẵn sàng");
        }
