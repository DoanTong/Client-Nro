	...
	public static void loadMapFromResource(int mapID)
{
    ...
    // Không có trong resource gốc (map custom của private server)
    // -> thử load từ cache mà server đã gửi trước đó
    sbyte[] cached = Rms.loadRMS("NRmaptile_" + mapID);
    if (cached == null || cached.Length < 2)
    {
        throw new Exception("Không có map local cho mapID=" + mapID);
    }

    myReader r = new myReader(cached);
    tmw = r.readUnsignedByte();
    tmh = r.readUnsignedByte();
    int tileCount = tmw * tmh;
    maps = new int[tileCount];
    for (int i = 0; i < tileCount; i++)
    {
        maps[i] = r.readUnsignedByte();
    }
    types = new int[maps.Length];
}
