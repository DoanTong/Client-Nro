...
public class mFont
{
    ...
    private static void ApplyGuiStyleState(GUIStyleState state, Color color)
    {
        if (state == null)
        {
            return;
    ...
    private void LockGuiStyleStates(GUIStyle style)
    {
        ApplyGuiStyleState(style.normal, color1);
        ApplyGuiStyleState(style.hover, color1);
        ApplyGuiStyleState(style.active, color1);
        ApplyGuiStyleState(style.focused, color1);
        ApplyGuiStyleState(style.onNormal, color1);
        ApplyGuiStyleState(style.onHover, color1);
        ApplyGuiStyleState(style.onActive, color1);
        ApplyGuiStyleState(style.onFocused, color1);
    }
