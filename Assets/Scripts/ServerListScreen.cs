...
public class ServerListScreen : mScreen, IActionListener
{
	...
	public static string ListIP = null;

	public static string linkDefault = "NRO:" + LinkIP + ":" + PortIP + ":0,0,0";
	...
	private void initCommand()
	{
		...
		cmd = new Command[(Main.isIPhone || mGraphics.zoomLevel <= 1) ? (4 + nCmdPlay) : (4 + nCmdPlay)];
			...
			switch (i)
			{
				...
				case 3:
					...
					else
					{
                        ...
                        cmd[3] = new Command(mResources.changeIP, this, 140, null);
                    }
					break;
				/*case 4:
					...
					break;*/
				case 4:
					cmd[4] = new Command(mResources.changeIP, this, 140, null);
					break;
			}
			cmd[i].y = num;
			cmd[i].setType();
	...
	public override void paint(mGraphics g)
	{
        ...
        if (isChangeIP) {
			...
			// Calculate layout similar to LoginScr
			int yLog = GameCanvas.hh - 30;
			num2 = 4;
			...
			int xLog = GameCanvas.w / 2 - num3 / 2;
			...
			if (GameCanvas.h > 160 && ModFunc.imgLogoBig != null) {
				ModFunc.PaintLogoGif(g, GameCanvas.hw, tfIP.y - 50, 3);
			}

            // Sync field positions
			tfIP.x = xLog + 10;
			tfIP.y = yLog + 20;
			tfPort.x = xLog + 10;
			tfPort.y = yLog + 55;
			...
			if (GameCanvas.w < 176) {
				mFont.tahoma_7b_green2.drawString(g, "IP:", tfIP.x - 35, tfIP.y + 7, 0);
				mFont.tahoma_7b_green2.drawString(g, "Port:", tfPort.x - 35, tfPort.y + 7, 0);
			}

			// Draw Header/Footer like LoginScr
			int numY = tfIP.y - 50;
			if (GameCanvas.h <= 220) numY += 5;
			...
			{
				mFont.tahoma_7_white.drawStringBorder(g, linkweb, GameCanvas.w - 2, 2, 1, mFont.tahoma_7_grey);
			}
		...
		if (SplashScr.imgLoadingBN != null)
		{
             g.drawImage(SplashScr.imgLoadingBN, GameCanvas.w / 2, GameCanvas.h / 2, 3);
		}
		else if (!loadScreen)
		{
			g.setColor(0);
	...
	{
		if (isChangeIP) {
			if (tfIP != null) tfIP.update();
			if (tfPort != null) tfPort.update();
			base.update();
			return;
		}
	...
	public override void keyPress(int keyCode)
	{
		if (isChangeIP)
		{
			if (tfIP != null && tfIP.isFocus)
			{
				tfIP.keyPressed(keyCode);
			}
			else if (tfPort != null && tfPort.isFocus)
			{
				tfPort.keyPressed(keyCode);
			}
		}
	...
	public override void updateKey()
	{
		...
		if (GameCanvas.isTouch)
		{
			...
			for (int i = 0; i < cmd.Length; i++)
			{
				...
				if ((testConnect == -1 || testConnect == 0) && isAutoConect)
				{
		...
		switch (idAction)
		{
			...
			case 140: // Change IP
				...
				if(tfIP == null) {
					...
					tfIP.x = xLog + 10;
					tfIP.y = yLog + 20;
					tfIP.width = ((GameCanvas.w < 200) ? 140 : 160);
					tfIP.height = mScreen.ITEM_HEIGHT + 2;
					tfIP.name = "IP";
					
					tfPort = new TField();
					tfPort.x = xLog + 10;
					tfPort.y = yLog + 55;
					tfPort.width = ((GameCanvas.w < 200) ? 140 : 160);
					tfPort.height = mScreen.ITEM_HEIGHT + 2;
					tfPort.name = "Port";
					tfPort.setIputType(TField.INPUT_TYPE_NUMERIC);
					tfPort.isPaintCarret = true;
					tfIP.isPaintCarret = true;
				}
				tfIP.setText(LinkIP);
				tfPort.setText(PortIP);
				tfIP.isFocus = true;
				tfPort.isFocus = false;
				
				center = new Command(mResources.OK, this, 141, null);
				right = new Command(mResources.CANCEL, this, 142, null);
				left = null;
				break;
			case 141: // Save IP
				LinkIP = tfIP.getText();
				...
				left = null;
				right = null;
				GameCanvas.startOK("Vui lòng khởi động lại game để cập nhật IP mới.", 8885, null);
                break;
			case 142: // Cancel IP
				isChangeIP = false;
				center = null;
